...
class CyrillicDatasetProcessor(AbstractDatasetProcessor):
    ...
    def process_dataset(self, out_dir: str, img_dir: str, gt_file: str, dataset_name: str = "cyrillic") -> None:
        with tempfile.TemporaryDirectory() as data_dir:
            ...
            test_df["path"] = f"{img_dir}/{self.__dataset_name}_test_" + test_df.path
            train_df["path"] = f"{img_dir}/{self.__dataset_name}_train_" + train_df.path
            ...
            self.logger.info(f"{self.__dataset_name} char set: {repr(''.join(sorted(list(char_set))))}")
            self.logger.info(f"{self.__dataset_name} dataset length: train = {len(df[df.stage == 'train'])}; "
            ...
            for img_dir_name in ("train", "test"):
                ...
                for img_name in os.listdir(current_img_dir):
                    new_img_name = f"{self.__dataset_name}_{img_dir_name}_{img_name}"
