...
from data import CTCTokenizer, TransformerTokenizer, collate_fn
from model import CRNN
from utils import *
from utilsFolder.logger import get_logger
from utilsFolder.metrics import cer, wer, string_accuracy, levenshtein_distance
...
def train_loop(data_loader, model, criterion_ctc, criterion_transformer, optimizer):
    ...
    for index, data in enumerate(data_loader):
        ...
        loss.backward()
        print('data num : '+ str(index) +'/'+ str(len(data_loader.dataset))+' total loss : '+str(losses['total'].avg)+ ' ctc loss : '+str(losses['ctc'].avg)+' transformer loss : '+str(losses['transformer'].avg))
